...
test_build_session_commandline()
{
	...
	ASSERT_STRING_EQ(out, buf);
	free(out);
	shell = "\"c:\\cygwin\\bash\" -c \"";
	out = build_session_commandline("c:\\cygwin\\bash", NULL, "internal-sftp \"arg1\\arg2\"");
	sprintf_s(buf, _countof(buf), "%s\"%s%s", shell, progdir, "\\sftp-server.exe\" \"arg1\\arg2\"");
	escape_chars(buf + strlen(shell));
	strcat(buf, "\"");
	ASSERT_STRING_EQ(out, buf);
	free(out);
	out = build_session_commandline("c:\\cygwin\\bash", NULL, "sftp-server \"arg1\\arg2 arg3\"");
	...
	ASSERT_STRING_EQ(out, buf);
	free(out);
	out = build_session_commandline("c:\\cygwin\\bash", NULL, "sCp \"arg1\\arg2 arg3\"");
