...
Describe "E2E scenarios for ssh client" -Tags "CI" {
   ...
   Context "$tC - Basic Scenarios" {
        ...
        It "$tC.$tI - remote echo command" {
            iex "ssh -F $ssh_config test_target echo 1234" | Should Be "1234"
        }
    }
    ...
    Context "$tC - exit code (exit-status.sh)" {
        ...
        It "$tC.$tI - various exit codes" {
            foreach ($i in (0,1,4,5,44)) {
                ssh -F $ssh_config test_target exit $i
    ...
    Context "$tC - Redirection Scenarios" {
        ...
        It "$tC.$tI - stdout to file" -skip:$skip {
            ssh -F $ssh_config test_target powershell get-process > $stdoutFile
        ...
        It "$tC.$tI - multiple double quotes in cmdline" {
            ...
            $o = ssh -F $ssh_config test_target `\`"cmd`\`" /c `\`"echo hello`\`"
            $o | Should Be "hello"
        }
        ...
        It "$tC.$tI - stdin from PS object" -skip:$skip {
            ...
            $o = $h | ssh -F $ssh_config test_target PowerShell -NoProfile -NonInteractive -ExecutionPolicy Unrestricted -EncodedCommand $EncodedText 2> $null
            $o | Should Be "8"
        }
        ...
        It "$tC.$tI - stream file in and out" -skip:$skip {
            ...
            $EncodedText =[Convert]::ToBase64String([System.Text.Encoding]::Unicode.GetBytes($str))
            # ignore error stream using 2> $null
            get-content $testsrc | ssh -F $ssh_config test_target PowerShell -NoProfile -NonInteractive -ExecutionPolicy Unrestricted -EncodedCommand $EncodedText 2> $null
            ...
            (ssh -F $ssh_config test_target powershell get-content $testdst1 -Encoding Ascii) | Set-Content $testdst2 -Encoding ASCII
    ...
    Context "$tC - configure powershell default shell Scenarios" {
        ...
        It "$tC.$tI - basic powershell" -skip:$skip {
            $o = ssh -F $ssh_config test_target Write-Output 1234
            $o | Should Be "1234"
        }

        It "$tC.$tI - basic in powershell cmdlet" -skip:$skip {
            $o = ssh -F $ssh_config test_target "cd `$env:ProgramFiles;pwd"
        ...
        It "$tC.$tI - powershell as default shell and double quotes in cmdline" {
            ...
            $o = ssh -F $ssh_config test_target echo "``\`"hello``\`""
            $o | Should Be "`"hello`""
        }
        ...
        It "$tC.$tI - multiple commands with double quotes in powershell cmdlet" -skip:$skip {
            ...
            $o = ssh -F $ssh_config test_target "dir \`"`$env:programfiles\`";cd \`"`$env:programfiles\`";pwd"
            $LASTEXITCODE | Should Be 0
        ...
        It "$tC.$tI - single quotes in powershell cmdlet" -skip:$skip {
            ...
            $o = ssh -F $ssh_config test_target "echo '`$env:computername'"
            $LASTEXITCODE | Should Be 0
    ...
    Context "$tC - configure cmd as default shell" {
        ...
        It "$tC.$tI - default shell as cmd" -skip:$skip {
            $o = ssh -F $ssh_config test_target where cmd
            $o | Should -BeLike "*cmd*"
        }
    ...
    Context "$tC - cmdline parameters" {
        ...
        It "$tC.$tI - verbose to file (-v -E)" {
            $o = ssh -v -E $logFile -F $ssh_config test_target echo 1234
            $o | Should Be "1234"
        ...
        It "$tC.$tI - cipher options (-c)" {
            #bad cipher
            iex "cmd /c `"ssh -c bad_cipher -F $ssh_config test_target echo 1234 2>$stderrFile`""
            $stderrFile | Should -FileContentMatch "Unknown cipher type"
            #good cipher, ensure cipher is used from debug logs
            $o = ssh -c aes256-ctr  -v -E $logFile -F $ssh_config test_target echo 1234
            $o | Should Be "1234"
        ...
        It "$tC.$tI - IP options - (-4) (-6)" {
            ...
            #-4
            $o = ssh -4 -v -E $logFile -F $ssh_config test_target echo 1234
            $o | Should Be "1234"
            $logFile | Should -FileContentMatch "[127.0.0.1]"
            #-4
            $o = ssh -6 -v -E $logFile -F $ssh_config test_target echo 1234
            $o | Should Be "1234"
        ...
        It "$tC.$tI - auto populate known hosts" {
            ...
            iex "cmd /c `"ssh -o UserKnownHostsFile=`"$kh`" -o StrictHostKeyChecking=no -F $ssh_config test_target hostname 2>&1`""
